...
// We need to use the number_of_words_in_block
// Using the tag mask
...
{
    number_of_memory_requests++; //********* once you are inside this function you will increment this number_of_memory_requests
    //cout << "hello";

    int tag;
    int word;
    int block_number;
    // int number_of_hits;  //********** number_of_hits is a global variable *******
    bool hits; //**** in order to print out if it is a hit (1) or miss (0) for each address*******
    ...
    // We know that the cache size 4KB 0r 4 *1024 = 4096 bytes
    // Split into three fields.
    // Tag, Line and Word offset



    // w would be 2 if we input a word id bit of 2 2^2
    block_offset = log2(number_of_words_in_block);


    //block number
    block_number = log2(address) - block_offset;


    // line number
    line_number = (4096/(2*exp(block_offset))); // ***** total number of lines= cache size/ size of the block or number_of_words_in_block*****
    //number_of_lines_in_cache++;

    // Int number to shift
    number = (log2(line_number)+ log2(number_of_words_in_block)); // ****instead of divide its a plus*******

    // This way we find the tag
    // (This thing needs to be in a bracket) in order to shift.
    tag = (address & tag_mask) >> number;

    //****** We also need to separate the line bits and the word bits from the address **********//

    i = (address & line_mask) >> block_offset; //
    int j = address & word_mask;
    //********* *******************************

    cout << address<<"\t";
    // We double if this tag is in the array.
    ...
    if (tags[i] != -1 && tags[i] == tag)
    {
          ...
          //if (tags[i] == tag)
                //{
                    //cout << "Hello";
                    hits = true;// cache is loaded with data
                    //cout << "yes";     
                    number_of_hits++;
                //}   
        ...
        for (int j = 0; j < number_of_words_in_block ; j++) // **** why (- block_number)
        {
            blocks[i,j] = memory[address]; // why divide by 256
            address++;
        }
    ...
    cout << "hit/miss  " << hits << "\t tag \t" << tag << endl;
    //cout << number_of_hits << "\n";
    
   

    // We make the for loop in order to add the tag and the number of words in a block to the blocks array
return hits; // when you put the return inside the if else, it was not printing our cout
}
...
cout << "\n" << number_of_hits << "\n";
cout << number_of_memory_requests << "\n";


double number = (double)number_of_hits/number_of_memory_requests;
cout << number <<"\n";
double hits_percentage ;



return hits_percentage = (number)*100; // I think i figured it out
...
#define SEQ5
    ...
    // Trying to print the tags array
