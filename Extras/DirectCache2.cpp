...
bool DirectCache::requestMemoryAddress(unsigned int address)
{
	...
	// We know that the cache size is 4096 bits or 256 words
	...
	// line number
	line_number = (2048/(2*exp(block_offset)));

	// Int number to shift
	number = (log2(line_number)/ log2(number_of_words_in_block));
	...
	if (tags[i] != -1)
	{
		...
		for (int j = 0; j < number_of_words_in_block - block_number; j++)
		{
		blocks[i,j] = memory[address]/256;
		address++;
		}
		number_of_hits = false;
		tags[i] = tag;
		return false;
...
unsigned int DirectCache::getPercentageOfHits(void)
{
	...
	for (int i = 0; number_of_hits; i++){
		if (number_of_hits == true)
		{
			number_of_hits++;
	...
	return hits_percentage = (number_of_hits/number_of_memory_requests)* 100;
